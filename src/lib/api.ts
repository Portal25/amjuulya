...
export const users = {
  ...
  async update(
    ...
    const { error } = await supabase()
      .from('users')
      // @ts-ignore – Supabase ssr types .update() as never without explicit schema match
      .update(updates)
      .eq('id', id);
...
export const tasks = {
  ...
  async uploadProofImages(
    ...
    if (urls.length === 0)
      throw new ApiError('UPLOAD_ERROR', 'Зураг байршуулахад алдаа гарлаа');
    const current = await tasks.getById(taskId);
    const merged = [...(current.proof_images || []), ...urls];
    const { error } = await db
      .from('tasks')
      // @ts-ignore – Supabase ssr types .update() as never without explicit schema match
      .update({ proof_images: merged })
      .eq('id', taskId);
  ...
  async uploadTaskImages(userId: string, files: File[]): Promise<string[]> {
    ...
    for (const file of files) {
      if (file.size > 5 * 1024 * 1024)
        throw new ApiError(
          'FILE_TOO_LARGE',
          'Зураг 5MB-аас хэтрэхгүй байх ёстой',
        );
      const ext = file.name.split('.').pop();
...
export const messages = {
  ...
  async send(
    ...
    if (!text && !imageUrl)
      throw new ApiError('EMPTY_MESSAGE', 'Мессеж хоосон байна');
    const { error } = await supabase()
      .from('messages')
      .insert({
        task_id: taskId,
...
export const transactions = {
  ...
  async deposit(userId: string, amount: number) {
    if (amount <= 0)
      throw new ApiError('INVALID_AMOUNT', 'Дүн 0-ээс их байх ёстой');
    const db = supabase();
...
export const disputes = {
  ...
  async resolve(
    disputeId: string,
    resolution: 'refund_user' | 'pay_worker' | 'split',
    notes?: string,
  ) {
    // @ts-ignore – Supabase ssr types rpc args as never without explicit schema
